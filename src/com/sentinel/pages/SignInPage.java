...
public class SignInPage extends WebDriverTestBase {
	...
	public void verifyFaviconPageTitle( ) {
		...
		try {
			...
			// stringValue = driver.getTitle() + "_demo";
			stringValue = driver.getTitle();
			// Verify Page Title
			Assert.assertTrue(WebDriverTestBase.driver.getTitle().equals(stringValue),
			...
			/*
			 * Assert.assertTrue(
			 * !WebDriverTestBase.driver.getTitle().equals(WebDriverTestBase.
			 * config.getProperty("projectTitle")), "Page not dispalyed as " +
			 * WebDriverTestBase.config.getProperty("projectTitle") +
			 * " . At Present it is displayed as - " +
			 * WebDriverTestBase.driver.getTitle());
			 */
			genericFunctions.logEvent("Verified Page Title..");
			// Verify Page Favicon
			stringValue = getFaviconIcon().getAttribute("href");
			Assert.assertTrue(genericFunctions.verifyLink(stringValue) == 200,
	...
	public void verifyLogoSignUpPanel() {
		try {
			...
			Assert.assertTrue(getLeftPanePoweredByLogo().isDisplayed(), "'Powered By' pane is not Displayed ...");
			genericFunctions.logEvent("Verified Powered by left side pane is Displayed Title..");
			Assert.assertTrue(getRightPaneSignIn().isDisplayed(), "'Sign In' pane is not Displayed ...");
	...
	public void verifySignInBoxElementsDisplayed() {
		try {
			...
			Assert.assertTrue(getTitleSignInBox().getText().equals("Sign In"),
					"Sign In Box Title is not expected Equal to 'Sign In'");
			Assert.assertTrue(getTextboxUsername().getAttribute("placeholder").equals("Enter Username"),
					"PlaceHolder does not contain the expected text 'Enter Username' .");
			Assert.assertTrue(getTextboxPassword().getAttribute("placeholder").equals("Enter Password"),
					"PlaceHolder does not contain the expected text 'Enter Password' .");
			Assert.assertTrue(!getButtonLogIn().isEnabled(), "The Submit Button is Enabled !!!!");
			Assert.assertTrue(getLinkForgetUsernameOrPassword().isDisplayed(),
		...
		try {
			...
			case "verifyErrorMessages":
				...
				while (i < data.readData(stringValue, fileNameAndExtension).size()) {
					...
					Assert.assertTrue(getToasterMessage().getText().equals("Invalid Credentials! Please try again."),
			...
			case "verifyMaxLimitValidUsernameInvalidPassword":
				...
				while (intValue >= 0) {
					...
					getButtonLogIn().click();
					Thread.sleep(800);
					Assert.assertTrue(getToasterMessage().isDisplayed(), "Error Message is not Displayed.");
					if (intValue != 0) {
						stringValue = "Invalid Credentials! Please try again. You have " + intValue + " attempt left.";

						Assert.assertTrue(getToasterMessage().getText().equals(stringValue),
	...
	public void verifyBlankSignInErrorMessages() {
		...
		try {
			...
			/*
			 * getTextboxUsername().click(); getTextboxPassword().click();
			 * getTextboxUsername().click();
			 * Assert.assertTrue(getErrorMessageblankUsername().isDisplayed(),
			 * "Blank Username Error Message is not dispayed .");
			 * Assert.assertTrue(getErrorMessageblankPassword().isDisplayed(),
			 * "Blank Password Error Message is not dispayed .");
			 * Assert.assertTrue(getErrorMessageblankUsername().getText().
			 * equals("Username is required"),
			 * "Blank Username Error Message is different than expected .");
			 * Assert.assertTrue(getErrorMessageblankPassword().getText().
			 * equals("Password is required"),
			 * "Blank Password Error Message is  different than expected .");
			 * genericFunctions.logEvent(
			 * "Verified Blank Username and Password Error Messages ....");
			 */} catch (Throwable T) {
	...
	public void verifyPaneTitle() {
		try {
			...
			Assert.assertTrue(getLabelForgotUsernamePassword().isDisplayed(),
	...
	public void verifyTextBoxesAndTextPlaceholders() {
		try {
			...
			Assert.assertTrue(getTextboxEnterEmailAddress().isDisplayed(),
					"'Enter Email Address' textbox is not displayed .");

			Assert.assertTrue(getTextboxEnterMobileNumber().isDisplayed(),
			...
			Assert.assertTrue(getTextboxEnterEmailAddress().getAttribute("placeholder").equals("Enter Email Address"),
					"'Enter Email Address' placeholder text is not displayed .");
			Assert.assertTrue(getTextboxEnterMobileNumber().getAttribute("placeholder").equals("Enter Mobile Number"),
	...
	public void verifyDNDLogoAndToolTip() {
		try {
			...
			Assert.assertTrue(getLogoDnd().isDisplayed());
			...
			stringValue = getTooltipDnd().getAttribute("data-original-title").replace(" <br/><strong> ", "")
					.replace("</strong>", "");
			Assert.assertTrue(stringValue.equals(config.getProperty("dndToolTip")),
			...
			while (i < data.readData(stringValue, fileNameAndExtension).size()) {
				while (j < buttonList.size()) {
					...
					if (buttonList.get(j).getText().equals("SUBMIT")) {
						GenericFunctions.highlightElement(buttonList.get(j));
						Assert.assertTrue(!buttonList.get(i).isEnabled(),
		...
		try {
			for (int i = 0; i < data.readData(stringValue, fileNameAndExtension).size(); i++) {
				...
				if (getButtonSubmit().isEnabled()) {
					Assert.assertTrue(getButtonSubmit().isEnabled(), "The Submit button is not enabled !!!");
					getButtonSubmit().click();
				}

				Assert.assertTrue(getErrorMessageProperEmailID().getText().equals("Please Enter Proper Email Address"),
			...
			getButtonSubmit().click();
			Assert.assertTrue(getErrorMessageProperEmailID().getText().equals("Please Enter Proper Mobile Number"),
	...
	public void verifyWithNonRegisteredEmailAndMobileNumber() {
		try {
			...
			Assert.assertTrue(getToasterMessageInvalidEmailOrPassword().isDisplayed(),
	...
	public void VerifyEmailedUserNameWithDB(String stringValue, String dbValue) {
		try {
			Assert.assertTrue(stringValue.trim().equals(dbValue.trim()), stringValue
	...
	public void verifyEnterOneTimePasswordLayout() {
		try {
			...
			// OTP Side Page Title
			Assert.assertTrue(getTitleEnterOneTimePassword().getText().trim().equals("Enter One Time Password"),
			 ...
			 * );
			 */
			Assert.assertTrue(getAlertInfoEnterOneTimePassword().isDisplayed(), "OTP Info text is not displayed");

			// Enter OTP textbox verification
			Assert.assertTrue(getTextboxEnterOTP().getAttribute("placeholder").trim().equals("Enter OTP"),
					"Enter Otp Text Box Placeholder is not equal to as expected , i.e,. 'Enter OTP'...");

			Assert.assertTrue(getTextboxEnterOTP().isDisplayed(), "Enter Otp Text Box is not Displayed...");

			// Resend Button Verification getButtonResend()
			Assert.assertTrue(getButtonResend().isDisplayed(), "Resend Button is not Displayed.");
			Assert.assertTrue(getButtonResend().isEnabled(), "Resend Button is not Enabled.");
			Assert.assertTrue(getButtonResend().getText().trim().equals("Resend"),
					"Resend Button text is not expected ,i.e,. 'Resend'.");

			// Small INfo verification LOC_INFO_ABOUTOTP_XPATH
			Assert.assertTrue(getInfoAboutOTP().isDisplayed(),
					"Small OTP Info beneath the 'Resend' Button is not Displayed.");

			Assert.assertTrue(getInfoAboutOTP().getText().trim().equals("OTP will expire in 10 minutes."),
					"OTP INFO text is not expected ,i.e,. ' OTP will expire in 10 minutes.'.");

			// SUbmit Button getButtonOTPSubmit()
			Assert.assertTrue(getButtonOTPSubmit().isDisplayed(), "Submit Button is not Displayed.");
			Assert.assertTrue(!getButtonOTPSubmit().isEnabled(), "Submit Button is  Enabled.");
			Assert.assertTrue(getButtonOTPSubmit().getText().trim().equals("SUBMIT"),
					"Button text is not expected ,i.e,. ' SUBMIT'.");

			// Cancel Button getButtonOTPCancel()
			Assert.assertTrue(getButtonOTPCancel().isDisplayed(), "Cancel Button is not Displayed.");
			Assert.assertTrue(getButtonOTPCancel().isEnabled(), "Cancel Button is not Enabled.");
			Assert.assertTrue(getButtonOTPCancel().getText().trim().equals("CANCEL"),
	...
	public void verifyEnteringOTPReceivedOnEmailed(String stringValue) {
		try {
			...
			Assert.assertTrue(getToasterSuccessmessageOTPSent().isDisplayed(),
					"Toaster Success Message is not displayed..");
			Assert.assertTrue(getToasterSuccessmessageOTPSent().getText().trim().equals("Your OTP is verified."),
	...
	public void verifyNewPasswordLayout() {
		try {
			...
			Assert.assertTrue(getTitleNewPassword().isDisplayed(), "'New Password' title is not displayed..");
			Assert.assertTrue(getTitleNewPassword().getText().trim().equals("New Password"),
					"'New Password' title literal is not expected . At present it is dispayed as - "
							+ getTitleNewPassword().getText().trim());

			// New Password Info
			Assert.assertTrue(getAlertInfoPassword().isDisplayed(), "'New Password' Alert Info is not displayed..");

			// New Password Text Box Validations
			Assert.assertTrue(getTextboxEnterNewPassword().isDisplayed(), "New Password textbox is not displayed..");
			Assert.assertTrue(
			...
			// New Password Text Box Validations
			Assert.assertTrue(getTextboxEnterConfirmNewPassword().isDisplayed(),
			...
			Assert.assertTrue(getButtonNewPasswordSubmit().isDisplayed(), "Submit Button is not displayed..");
			Assert.assertTrue(!getButtonNewPasswordSubmit().isEnabled(), "Submit Button is  Enabled..");
			Assert.assertTrue(getButtonNewPasswordSubmit().getText().trim().equals("SUBMIT"),
			...
			Assert.assertTrue(getButtonNewPasswordCancel().isDisplayed(), "Cancel Button is not displayed..");
			Assert.assertTrue(getButtonNewPasswordCancel().isEnabled(), "Cancel Button is not Enabled..");
			Assert.assertTrue(getButtonNewPasswordCancel().getText().trim().equals("CANCEL"),
	...
	public String verifyNewPasswordConfirmPasswordFuntionality(String... args) {
		try {
			...
			if (args[0].equals("change to default password")) {
				...
				Assert.assertTrue(genericFunctions.verifyLink(getLinkClickHere().getAttribute("href")) == 200,
	...
	public void verifySuccessfullyLoginOnPasswordChange(String firstName, String newPassword) {
		try {
			...
			Assert.assertTrue(getLogoUpperLeftCornerHomePage().isDisplayed(), "User Not Logged IN . ");
	...
	public void verifyBlankOTPErrorMessage() {
		try {
			...
			Assert.assertTrue(getErrormessageBlankOTP().isDisplayed(), "Blank Otp error message is not displayed..");
			Assert.assertTrue(getErrormessageBlankOTP().getText().equals("Otp is required"),
	...
	public void verifyOTPCharLimitErrorMessage() {
		try {
			...
			getButtonOTPSubmit().click();
			Assert.assertTrue(getCharLimitErrorMessage().isDisplayed(),
					"Character Limit Error Message is not displayed..");

			Assert.assertTrue(getCharLimitErrorMessage().getText().trim().equals("The otp value must be 6 digits."),
	...
	public void verifyInvalidOTPErrorMessage() {
		try {
			...
			getButtonOTPSubmit().click();
			Assert.assertTrue(getErrorMessageInvalidOTP().isDisplayed(),
					"Invalid OTP Error Message is not displayed..");
			Assert.assertTrue(getErrorMessageInvalidOTP().getText().trim().equals("OTP is invalid. Please try again."),
	...
	public void verifyChangePasswordAndConfirmPasswordBlankErrorMessages() {
		try {
			...
			Assert.assertTrue(getErrorMessageBlankChangePassword().isDisplayed(),
					"Invalid OTP Error Message is not displayed..");
			Assert.assertTrue(getErrorMessageBlankConfirmPassword().isDisplayed(),
					"Invalid OTP Error Message is not displayed..");

			Assert.assertTrue(getErrorMessageBlankChangePassword().getText().trim().equals("Password is required"),
	...
	public void verifyChangePasswordAndConfirmPasswordInvalidErrorMessages() {
		try {
			...
			Assert.assertTrue(getErrorMessageInvalidConfirmPassword().isDisplayed(),
					"Invalid Confirm Password Error Message is not dispayed..");
			Assert.assertTrue(getErrorMessageInvalidConfirmPassword().getText().equals("Password is not matching."),
					"Invalid Confirm Password Error Message is not as expected ..Expected Error Message is 'Password is not matching.' .");
			Assert.assertTrue(!getButtonNewPasswordSubmit().isEnabled(), "New Password Submit Button is enabled..");
		...
		try {
			while (intValue != Integer.parseInt((config.getProperty("resetPasswordMailSentMaximumValue") + 1))) {
				...
				if (intValue < Integer.parseInt(config.getProperty("resetPasswordMailSentMaximumValue"))) {
					...
					Assert.assertTrue(getToasterMessageResetPasswordMaximumLimit().isDisplayed(),
	...
	public void verifyOTPcountInDatabaseIsMaximum(int intValue) {
		try {
			Assert.assertTrue(intValue == Integer.parseInt(config.getProperty("resetPasswordMailSentMaximumValue")),
		...
		try {
			...
			while (intValue <= (Integer.parseInt(config.getProperty("resendOTPMaximumValue")))) {
				if (intValue < Integer.parseInt(config.getProperty("resendOTPMaximumValue"))) {
					...
					Assert.assertTrue(getToasterErrormessageOTPSent().isDisplayed(),
	...
	public void verifyDbValuesPostInvalidPasswordAttempts(int intValue, int intValue1) {
		try {
			Assert.assertTrue(intValue == Integer.parseInt(config.getProperty("maximumInvalidLogins")),
			...
			Assert.assertTrue(intValue1 == 0, "'tblcontact' table 'active' coloumn value i,.e,. " + intValue1
					+ " does not match with the expected value . i.,e,. - 0");
			genericFunctions.logEvent(
	...
	public void testProirityFourTC() {
		try {
			 Assert.assertTrue(1 == 1);
		} catch (Exception e) {
