...
public class CreateFacilityTestSuite extends WebDriverTestBase {
	...
	public void verifyFacilityCreateLayout() throws InterruptedException {
		...
		createFacilityFormPage.testProirityOneTC();

		/*
		 * stringValue = signIn.resetToDefaultPassword(
		 * "change to default password"); // stringValue = "Admin@123"; // Login
		 * the application signIn.loginApplication(stringValue); // Navigate to
		 * Create Facility Page homePage.navigateToFacilityPage(
		 ...
		 * createFacilityFormPage.verifyRemainingSalesTaxTextbox(); // logout
		 * the Application signIn.logoutTheApplication();
		 */
	}
	...
	public void verifyFacilityFormCreated() throws InterruptedException {
		 ...
		 * CHANGE THE PASSWORD TO DEFAULT
		 */
		facilityPage.testProirityThreeTC();

		/*
		 * stringValue = signIn.resetToDefaultPassword(
		 * "change to default password"); // stringValue = "Admin@123"; // Login
		 * the application signIn.loginApplication(stringValue);
		 * homePage.navigateToFacilityPage(config.getProperty(
		 * "navigateToCreateFacilitypage")); // fill the form with all the
		 * fields and verify in the database linkedHashMap =
		 * createFacilityFormPage.saveCreateFacilityForm("All Fields");
		 * facilityPage.verifyAddedFacility(linkedHashMap); // logout the
		 * Application signIn.logoutTheApplication();
		 */
	}
