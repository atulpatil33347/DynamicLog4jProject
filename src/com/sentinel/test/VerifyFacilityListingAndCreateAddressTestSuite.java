...
public class VerifyFacilityListingAndCreateAddressTestSuite extends WebDriverTestBase {
	...
	@Test(enabled = true, groups = { "Sprint3" }, priority = 2, description = "Verifing Address page Layout.")
	public void verifyAddressPageLayout() throws InterruptedException {
		 ...
		 * CHANGE THE PASSWORD TO DEFAULT
		 */
		addressPage.testPTwoTC();
		/*
		 * stringValue = signIn.resetToDefaultPassword(
		 * "Don't Change the Password"); // stringValue = "Admin@123"; // Login
		 * the application signIn.loginApplication(stringValue);
		 * homePage.navigateToFacilityPage("Navigate to Create Form Page");
		 * linkedHashMap = createFacilityFormPage.saveCreateFacilityForm(
		 * "Mandatory Fields");
		 * addressPage.navigateToViewFacilityPage(linkedHashMap); arrayListValue
		 * = projectDatabaseQueries.getFacilityDataForFirstRow();
	...
	public void verifyFacilityFilter() throws InterruptedException {
		...
		createFacilityFormPage.testPSixTC();
		 ...
		 * homePage.navigateToFacilityPage("Navigate to Create Form Page");
		 * linkedHashMap = createFacilityFormPage.saveCreateFacilityForm(
		 * "Mandatory Fields"); arrayListValue =
		 * projectDatabaseQueries.getFacilityDataForFirstRow();
		 * createFacilityFormPage.verifyFacilityListingFilterButtonAndLabel();
		 * createFacilityFormPage.verifyPopUpElements();
		 * createFacilityFormPage.verifyDropdowns(); // Search by Name
		 * arrayListValue = projectDatabaseQueries.getFacilityWithCityState();
		 * createFacilityFormPage.verifySearchFunctionalityUsingName(
		 * arrayListValue, "Name"); // Search by State arrayListValue =
		 * projectDatabaseQueries.getFacilityWithCityState();
		 * createFacilityFormPage.verifySearchFunctionalityUsingName(
		 * arrayListValue, "State"); // Search by City arrayListValue =
		 * projectDatabaseQueries.getFacilityWithCityState();
		 * createFacilityFormPage.verifySearchFunctionalityUsingName(
		 * arrayListValue, "City");
		 * 
